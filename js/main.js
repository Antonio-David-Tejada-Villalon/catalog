...
.then(d => {
	...
	for(let x of d){
		...
		if(name.indexOf(text) !== -1){
			result.innerHTML += `<a href="#${x.TITULO}" class="list-group-item d-flex justify-content-between align-items-start w-auto"> <span class="badge bg-primary sc1">${x.TITULO}</span> Precio: <span class="badge bg-secondary sc1">$ ${x.IMPORTE}</a>`
		}
	}
	if(result.innerHTML === ''){
